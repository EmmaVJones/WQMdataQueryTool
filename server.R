...
shinyServer(function(input, output, session) {
  ...
  observeEvent(nrow(reactive_objects$stationInfo) > 0, {
    ...
    reactive_objects$stationInfoFin <- stationInfoConsolidated(pool, input$station, WQM_Station_Full_REST(), WQM_Stations_Spatial)
...
### Add in Benthic Data
  ...
  observe({ req(nrow(reactive_objects$stationInfoBenSamps) > 0)
    ...
    reactive_objects$SCI_filter <- filter(VSCIresults, BenSampID %in% filter(reactive_objects$stationBasinEco, ! EPA_ECO_US_L3CODE %in% c(63,65))$BenSampID) %>%
      bind_rows(
        filter(VCPMI63results, BenSampID %in% filter(reactive_objects$stationBasinEco,  EPA_ECO_US_L3CODE %in% c(63) | str_detect(Basin, "Chowan"))$BenSampID)  ) %>%
      bind_rows(
        filter(VCPMI65results, BenSampID %in% filter(reactive_objects$stationBasinEco,  EPA_ECO_US_L3CODE %in% c(65) & !str_detect(Basin, "Chowan"))$BenSampID)  ) %>%
      mutate(SeasonGradient = as.factor(paste0(Season, " (",Gradient,")")),
             SeasonGradientColor = case_when(SeasonGradient == "Spring (Riffle)" ~  "#66C2A5",
                                             ...
                                             TRUE ~ as.character(NA)) ) %>%
      left_join(dplyr::select(reactive_objects$stationInfoFin, Sta_Id , Sta_Desc) %>% 
  ...
  ### SCI table
  output$SCIresultsTable <- DT::renderDataTable({    req(reactive_objects$SCI_filter)
  ...
  output$multistationDateRangeFilter_ <- renderUI({ req(nrow(reactive_objects$multistationFieldData) > 0)
    ...
    observe({ req(nrow(reactive_objects$multistationInfoBenSamps) > 0)
      ...
      reactive_objects$multistationSCI_filter <- filter(VSCIresults, BenSampID %in% filter(reactive_objects$multistationBasinEco, ! EPA_ECO_US_L3CODE %in% c(63,65))$BenSampID) %>%
        bind_rows(
          filter(VCPMI63results, BenSampID %in% filter(reactive_objects$multistationBasinEco,  EPA_ECO_US_L3CODE %in% c(63) | str_detect(Basin, "Chowan"))$BenSampID)  ) %>%
        bind_rows(
          filter(VCPMI65results, BenSampID %in% filter(reactive_objects$multistationBasinEco,  EPA_ECO_US_L3CODE %in% c(65) & !str_detect(Basin, "Chowan"))$BenSampID)  ) %>%
        mutate(SeasonGradient = as.factor(paste0(Season, " (",Gradient,")")),
               SeasonGradientColor = case_when(SeasonGradient == "Spring (Riffle)" ~  "#66C2A5",
                                               SeasonGradient == "Spring (Boatable)" ~  "#66C2A5",
                                               SeasonGradient == "Spring (MACS)" ~  "#66C2A5",
                                               ...
                                               SeasonGradient == "Fall (MACS)" ~ "#8DA0CB",
                                               TRUE ~ as.character(NA)) ) %>%
        left_join(dplyr::select(reactive_objects$multistationInfoFin, Sta_Id , Sta_Desc) %>%
                    distinct(Sta_Id, .keep_all = T),
                  by = c('StationID'= 'Sta_Id')) %>%
        dplyr::select(StationID, Sta_Desc, BenSampID, `Collection Date`, RepNum, SCI, `SCI Score`, `SCI Threshold`,`Sample Comments`:Season, everything())     })
    ...
    # ### SCI plotly
    # output$multistationSCIplot <- renderPlotly({    req(reactive_objects$multistationSCI_filter)
    #   if(unique(reactive_objects$SCI_filter$SCI) == 'VSCI'){sciLimit <- 60} else {sciLimit <- 40}
    # 
    #   SCIresults <- reactive_objects$multistationSCI_filter
    #   SCIresults$SeasonGradient <- factor(SCIresults$SeasonGradient,levels=c("Spring (Riffle)",
    #                                                                          "Spring (Boatable)",
    #                                                                          "Spring (MACS)",
    #                                                                          "Outside Sample Window (Riffle)",
    ...
    ### SCI table
    output$multistationSCIresultsTable <- DT::renderDataTable({    req(reactive_objects$multistationSCI_filter)
      datatable(reactive_objects$multistationSCI_filter  %>% mutate(`Collection Date` = as.Date(`Collection Date`)),
                rownames = F, escape= F, extensions = 'Buttons', selection = 'none',
                options = list(dom = 'Bit', scrollX= TRUE, scrollY = '250px',
                               pageLength= nrow(reactive_objects$multistationSCI_filter), buttons=list('copy','colvis')))  })


    ### Habitat Tab
    # output$multistationtotalHabitatPlot <- renderPlotly({req(nrow(reactive_objects$multistationTotalHabitat)>0)
    #   reactive_objects$stationTotalHabitat %>%
    #     plot_ly( x = ~`Collection Date`, y = ~`Total Habitat Score`, type = 'bar',
    #              color = ~Season, width = 0.5, stroke = list(color = 'rgb(0, 0, 0)', width = 3),
    #              #marker = list(line = list(width = 1.5)),
    ...
    output$multistationHabitatResultsTable <- DT::renderDataTable({req(nrow(reactive_objects$multistationTotalHabitat)>0)
      datatable(reactive_objects$multistationTotalHabitat %>%
                  mutate(`Collection Date` = as.Date(`Collection Date`)),
                rownames = F, escape= F, extensions = 'Buttons',
                options = list(dom = 'Bift', scrollX= TRUE, scrollY = '300px',
                               pageLength = nrow(reactive_objects$multistationTotalHabitat), buttons=list('copy','colvis')))  })

    ## LRBS
    output$multistationLRBSTable_habitatTab <- renderDataTable({ req(reactive_objects$multistationAnalyteDataUserFilter)
      z <- filter(LRBS, StationID %in% unique(reactive_objects$multistationSelection$Sta_Id) &
                    between(as.Date(Date), as.Date(input$multistationDateRangeFilter[1]), as.Date(input$multistationDateRangeFilter[2]) ) ) %>%
        mutate(Date = as.Date(Date))
