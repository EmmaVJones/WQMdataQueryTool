metalsDataWQS1 <- multistationFieldAnalyte1 %>% 
    {if(! 'Uranium, total, ug/L' %in% names(multistationFieldAnalyte1))
...
stationMetalsDataWQS1 <- filter(metalsDataWQS1, Station_Id %in% '2-JKS028.69') %>% 
    left_join(WQSlookup, by=c('Station_Id' = 'StationID')) 




stationMetalsData <- stationMetalsDataWQS1 %>% 
  dplyr::select(Station_Id:RMK_Hardness)
CLASS <- unique(stationMetalsDataWQS1$CLASS)
ZONE <- unique(stationMetalsDataWQS1$ZONE)
PWS <- unique(stationMetalsDataWQS1$PWS)
WER <- 1
