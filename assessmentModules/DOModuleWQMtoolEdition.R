  ...
  tagList(
    wellPanel(
      ...
      fluidRow(column(2,uiOutput(ns('oneStationSelectionUI'))),
               ...
               column(2,helpText('WQS adjustment applies to this module instance and does not save WQS adjustments or carry over to pooled AU assessments.')),
               column(1),
               column(4,actionButton(ns('zoomPlotDO'),"Zoomed Plot by Sample Date",class='btn-block'))),
               # column(1),
               # column(2,actionButton(ns('reviewData'),"Review Raw Parameter Data",class='btn-block', width = '250px'))),
      helpText('All data presented in the interactive plot is raw data.'),
      #Rounding rules are appropriately applied to the assessment functions utilized by the application.'),
...
DOPlotlySingleStation <- function(input,output,session, AUdata, stationSelectedAbove){
  ...
  oneStation_original <- reactive({req(ns(input$oneStationSelection))
    filter(AUdata(),FDT_STA_ID %in% input$oneStationSelection) %>%
      ...
      # extra step since this isnt from conventionals data
      mutate(DO_mg_L = coalesce( FDT_DO_PROBE, FDT_DO_OPTICAL, FDT_DO_WINKLER)) %>% 
  ...
  observeEvent(input$zoomPlotDO,{
    showModal(modalDialog(
      ...
      hr(),
      radioButtons(ns('DOplotStyle'), 'Plot By:', choices = c('Depth vs Dissolved Oxygen', 'Temperature vs Dissolved Oxygen')),
  ...
  output$DOplotlyByDate <- renderPlotly({  req(oneSampleDate())
    ...
    if(input$DOplotStyle == 'Temperature vs Dissolved Oxygen'){
      plot_ly(data=dat)%>%
      ...
      plot_ly(data=dat)%>%
        add_lines(x=~`Dissolved Oxygen Min (mg/L)`,y=~FDT_DEPTH, mode='line',line = list(color = '#E50606'),
                  hoverinfo = "text",text= paste("DO Standard:",unique(dat$`Dissolved Oxygen Min (mg/L)`),' (mg/L)',sep=''), name = 'DO Standard') %>%
        add_markers(x= ~DO_mg_L, y= ~FDT_DEPTH,mode = 'scatter', name="Dissolved Oxygen",
                    color=~LakeStratification, #colors=c('#BF382A', '#0C4B8E'),
                    hoverinfo="text",text=~paste(sep="<br>",paste("Depth: ",FDT_DEPTH, "m"),
                                                 paste("Dissolved Oxygen:",DO_mg_L,"mg/L"),
                                                 paste("LakeStratification: ",LakeStratification)))%>%
